...
for element in direcciones:
    ...
    while i < (len(jugadores) - 1):
        with open("../data/ligaSantander1819.csv", "a") as toWrite:
            writer = csv.writer(toWrite, delimiter=",", lineterminator='\n')
            writer.writerow([soup_aux.find("div", class_="table-footer").find("a")["title"], jugadores[i].find("div", class_="rn_nummer").get_text(), jugadores[i+2].get_text(), 
                             jugadores[i].find("a", class_="spielprofil_tooltip").get_text().encode("utf-8"), jugadores[i].find_all(class_="zentriert")[1].get_text(),
                             jugadores[i].find_all(class_="zentriert")[2].find("img")["title"].encode("utf-8"), jugadores[i].find(class_="rechts hauptlink").get_text().encode("utf-8")])
        i += 9
